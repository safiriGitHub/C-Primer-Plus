/* mall.c -- 使用 Queue 接口和queue.c一起编译 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include "6-queue.h"
...
Item customertime(long when)
{
    Item cust = {when, rand()%3 + 1};
    return cust;
}

/* 
解释1
把rand() * x / RAND_MAX < 1做一个变形为：rand() / RAND_MAX < 1 / x
0~1之间的随机浮点数，小于1/2的概率是1/2，小于1/x的概率为1/x。即每隔x次，就会出现一次。
一个特殊取值是当 rand() / RAND_MAX == 1/x时， rand() * x / RAND_MAX == 1。
 */